...
func TestAnalyzerRunSkipsGeneratedGoTestMainPackage(t *testing.T) {
	...
	if analyzedPackages == 0 {
		t.Fatal(testExpectedAnalyzablePkg)
	}
...
func TestAnalyzerRunAllowsTaggedFileSelectorsFromGOFLAGS(t *testing.T) {
	resetProcessRepoValidationCacheForTesting()
	t.Cleanup(resetProcessRepoValidationCacheForTesting)

	repoRoot := t.TempDir()
	writeAnalyzerFixtureFile(t, repoRoot, goModFilename, testGoModSource)
	writeAnalyzerFixtureFile(
		t,
		repoRoot,
		testAllowlistPath,
		"version: 2\nentries:\n  - selector:\n      path: pkg/api/payload_custom.go\n      owner: Payload\n      category: \"*ast.MapType.Value\"\n      line: 5\n      column: 25\n    description: custom build-tag boundary\n",
	)
	writeAnalyzerFixtureFile(
		t,
		repoRoot,
		filepath.Join(testPkgDir, testDirAPI, testPayloadCustomFile),
		testCustomTaggedPayloadSrc,
	)

	applyBuildEnv(t, buildEnv{goflags: testGOFLAGSCustom})
	...
	pkgs := loadAnalyzerTestPackages(t, repoRoot)
	foundTaggedFile := false
	for _, pkg := range pkgs {
		for _, file := range pkg.CompiledGoFiles {
			if filepath.ToSlash(file) == filepath.ToSlash(filepath.Join(repoRoot, testPkgDir, testDirAPI, testPayloadCustomFile)) {
				foundTaggedFile = true
			}
		}
	}
	if !foundTaggedFile {
		t.Fatalf("expected tagged file to be part of the analyzed package set")
	}

	_, analyzedPackages, diagnosticCount := runAnalyzerPackages(t, cfg, pkgs)
	if analyzedPackages == 0 {
		t.Fatal(testExpectedAnalyzablePkg)
	}
	if diagnosticCount != 0 {
		t.Fatalf("expected tagged file allowlist to suppress diagnostics, got %d", diagnosticCount)
	}
}
